...
static void bcm2835_mbox_update(BCM2835MboxState *s)
{
    ...
    qemu_log_mask(LOG_GUEST_ERROR, "%s: update\n", __func__);
    ...
    for (n = 0; n < 2; n++) {
        set = false;
        qemu_log_mask(LOG_GUEST_ERROR, "%s: mbox %u cfg %x status %x cnt %u\n",
                      __func__, n, s->mbox[n].config, s->mbox[n].status, s->mbox[n].count);
        s->mbox[n].config &= ~ARM_MC_IHAVEDATAIRQPEND;
        if (!(s->mbox[n].status & ARM_MS_EMPTY)) {
            s->mbox[n].config |= ARM_MC_IHAVEDATAIRQPEND;
            if (s->mbox[n].config & ARM_MC_IHAVEDATAIRQEN) {
                set = true;
            }
        }
        qemu_log_mask(LOG_GUEST_ERROR, "%s: mbox %u set irq %u\n", __func__, n, set);
        qemu_set_irq(s->arm_irq[n], set);
    }
}
...
static void bcm2835_mbox_set_irq(void *opaque, int irq, int level)
{
    ...
    qemu_log_mask(LOG_GUEST_ERROR, "%s: irq %u\n", __func__, irq);
     ...
     */
#if 0
    if (irq == 161 && !s->mbox_irq_disabled) {
...
static uint64_t bcm2835_mbox_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    case 0xa0 ... 0xac: /* MAIL1_READ */
        if (s->mbox[1].status & ARM_MS_EMPTY) {
            res = MBOX_INVALID_DATA;
        } else {
            res = mbox_pull(&s->mbox[1], 0);
        }
        break;
                               ...
                               uint64_t value, unsigned size)
{
    ...
    case MAIL0_CONFIG:
        ...
        qemu_log_mask(LOG_GUEST_ERROR, "%s: mbox %u config updated: %x\n",
                      __func__, 0, s->mbox[0].config);
        break;

    case MAIL1_CONFIG:
        s->mbox[1].config &= ~ARM_MC_IHAVEDATAIRQEN;
        s->mbox[1].config |= value & ARM_MC_IHAVEDATAIRQEN;
        qemu_log_mask(LOG_GUEST_ERROR, "%s: mbox %u config updated: %x\n",
...
static void bcm2835_mbox_init(Object *obj)
{
    ...
    sysbus_init_irq(SYS_BUS_DEVICE(s), &s->arm_irq[0]);
    sysbus_init_irq(SYS_BUS_DEVICE(s), &s->arm_irq[1]);
...
static void bcm2835_mbox_realize(DeviceState *dev, Error **errp)
{
     ...
     */
    memory_region_init(&s->mbox_mr_real, OBJECT(dev), "bcm2835-mbox",
